...
for combination in itertools.product(*my_iter):
    #for combination2 in itertools.product(*reversed_iter):    
    counter = counter + 1
    #start_loop = timer()
    #print(combination)
    calc_time = timer()
    local_maxmin = simulate(d,combination,N)
...
# in double-check mode, new best result is checked again in the matrices   
    if local_maxmin.min() > (((stack[stack_size-1])[0]).min()*0.9): #factor 0.9 to avoid miss
        ...
        if local_maxmin.min() > (((stack[stack_size-1])[0]).min()*0.9): #factor 0.9 to avoid miss
            ...
            calc_time = timer()
            local_maxmin = simulate(d,combination,30*N)
            print('second double check (Nx30), took %3.3f'% (timer()-calc_time),'sec, result=', local_maxmin)
            ...
            if local_maxmin.min() > ((stack[stack_size-1])[0]).min():
                ...
                if local_maxmin.min() > ((stack[stack_size-1])[0]).min():
                    ...
                    stack[stack_size-1] = [local_maxmin,combination]
                    ...
                    if stack_counter == stack_size:
                        ...
                        while (i < stack_size):
                            if ((stack[i])[0]).min() > MaxMinPPD.min():
                                ...
                                #strong simulation
                                calc_time = timer()
                                val = simulate(d,(stack[i])[1],200000)
    ...
    if (counter % 1000 == 0):
        ...
        print('current position:', (np.around(combination,decimals=3)))
